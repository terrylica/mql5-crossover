//+------------------------------------------------------------------+
//|                                      CCI_Neutrality_ScoreOnly.mq5 |
//|                      Visual version - Neutrality Score ONLY       |
//+------------------------------------------------------------------+
#property copyright   "Terry Li"
#property link        "https://github.com/terrylica/mql5-crossover"
#property version     "3.00"
#property description "CCI Neutrality Score - Score line only (composite metric)"
...
//--- Set indicator levels (0.0 = neutral, 1.0 = perfect neutrality)
...
//--- Set indicator name
   IndicatorSetString(INDICATOR_SHORTNAME,
                      StringFormat("CCI Neutrality Score(%d,%d)", InpCCILength, InpWindow));
...
//--- Create CCI indicator handle
   ...
   PrintFormat("CCI Neutrality Score initialized: CCI=%d, W=%d",
...
//--- Release CCI handle
   ...
   PrintFormat("CCI Neutrality Score deinitialized, reason: %d", reason);
  }
   ...
   for(int i = start; i < rates_total && !IsStopped(); i++)
     {
      ...
      if(is_new_bar)
      ...
      else
        {
         ...
         if(i == last_processed_bar && BufScore[i] != EMPTY_VALUE)
           {
            // Remove old CCI contribution
            // (We can't retrieve old CCI value, so reconstruct from score)
            // This is a simplification - in production, store old CCI separately
           }
