...
#property description "Detects consecutive expansions/contractions in bar body size (v1.40)"
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Consecutive Pattern v1.40 (" + string(InpConsecutiveCount) + ")");
                ...
                const int &spread[])
{
   ...
   if(InpShowContractions)
   {
      for(int i = 1; i < rates_total - InpConsecutiveCount; i++)
      {
         ...
         // v1.30: Force remove bar coloring for contractions (keep circles only)
         // This overrides the color set by SetContractionSignal() above
         BufferColorIndex[i] = COLOR_NONE;
      }
   }
