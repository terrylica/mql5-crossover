...
> "It's not explicitly stated if the default path is also searched as a fallback, but given the user's error avalanche and others' experiences, it likely is not. In other words, `/include` _replaces_ the default include directory."
...
> "Use the `/include` parameter _sparingly_. In our automated build pipeline, we should avoid `/inc` unless absolutely necessary."
